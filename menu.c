...
// Gets the total submenus available under each 
// menu as defined in the menu struct 
...
void menuDisplay(int title_color) {
    ...
    for (int j = 0; j<getMenuTotal(currentMenu); j++) {
        ...
        if (j < getMenuTotalItems(currentMenu)) {
            ...
            if (1 != menus[menuI].menuLvl) { // see if menu is top- or sub-lvl
...
int menu() {
    ...
    int easterBlink = 0;
    int creditsToggle = 0;
    while (getch() != -1); // clear the buffer
    ...
    while (1) {
        ...
        if (!creditsToggle && (w >= MAXW && h >= MAXH)) {
            if (ARROW_DOWN == inp_char){
                ...
                moveUp();
                _change = 1;
            } else if (SPACE_KEY == inp_char || ENTER_KEY == inp_char) {
                selectItem();
                _change = 1;
            }
        }
